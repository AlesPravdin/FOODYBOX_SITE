...
class ControllerPaymentWebPay extends Controller {
	protected function index() {
		...
		$this->data['wsb_test'] = $this->config->get('webpay_test'); // 1 - for test

		if ($this->data['wsb_test'] == 1)
			$this->data['action'] = 'https://sandbox.webpay.by'; // for test
				else
			$this->data['action'] = 'https://secure.webpay.by'; // for real
		...
		foreach ($total_data as $total) {  
			...
			$this->data['wsb_discount_price'] = $this->data['wsb_discount_price'] + abs($total['value']);
		...
		if ($this->data['wsb_tax'] < 0)
		{
			$this->data['wsb_discount_price'] = $this->data['wsb_discount_price'] - $this->data['wsb_tax'];
			$this->data['wsb_tax'] = 0;
		}
