...
class ModelModuleProductOptionImagePro extends Model {
  ...
  public function getProductOptionImagesByValues($product_id) {
    ...
    foreach ($results as $result) {
      ...
      $images[0][] = array( 'image'             => $result['image']
                          , 'option_name'       => ''
                          , 'product_option_id' => 0
                          , 'srt'               => $result['sort_order']
                          , 'thumb'             => $result['image']
                          , 'value_name'        => ''
                          );
  ...
  public function addOptionImagesToProductImages($product_images_old, $product_id, &$product_images, $popup_width=false, $popup_height=false) {
    ...
    foreach (array_reverse($product_images_old) as $product_image) {
      if (!in_array($product_image['image'], $added_images)) {
        ...
        foreach ($product_images as &$product_image_new) {
          if ($product_image_new['image'] == $product_image['image']) {
            if ( !isset($product_image_new['product_image_id']) ) {
              $product_image_new['product_image_id'] = $product_image['product_image_id'];
            }
            break;
          }
        }
        unset($product_image_new);
      }
    }
