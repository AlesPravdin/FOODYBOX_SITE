...
/////////////////////////////////////////////////////////////////////////////
	public function getProductsByCategory($data = array()) {
		...
		if (!$product_data) {
			...
			$sql .= " WHERE pd.language_id = '" . (int)$this->config->get('config_language_id') . "' AND p.status = '1' AND p2c.category_id = '".$data['cat_id']."'AND p.date_available <= NOW() AND p2s.store_id = '" . (int)$this->config->get('config_store_id') . "'"; 
	...
	/* Get category name by category id for ByCategory Module */
	public function getByCategoryName($catId){
		$query = $this->db->query("SELECT name FROM " . DB_PREFIX . "category_description where category_id= " . $catId . " limit 1");
