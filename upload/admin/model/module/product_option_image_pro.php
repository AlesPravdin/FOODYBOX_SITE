...
class ModelModuleProductOptionImagePro extends Model {
  ...
  // for export
  public function getAllImages($add_products_without_options_images=false) {
    $query = $this->db->query(" SELECT POIP.product_id
                                      ,POV.option_value_id
                                      ,POIP.image
                                      ,POIP.sort_order
                                      ,OVD.name
                                FROM ".DB_PREFIX."poip_option_image POIP
                                    ...
                                    LEFT JOIN ".DB_PREFIX."option_value_description OVD
                                      ON (OVD.option_value_id = POV.option_value_id AND OVD.language_id = ".(int)$this->config->get('config_language_id').")
    ...
    if ($add_products_without_options_images) {
      $query = $this->db->query(" SELECT P.product_id, POV.option_value_id, '', 1, OVD.name
                                  FROM ".DB_PREFIX."product P
                                      ,".DB_PREFIX."product_option_value POV
                                      LEFT JOIN ".DB_PREFIX."option_value_description OVD
                                      ON (OVD.option_value_id = POV.option_value_id AND OVD.language_id = ".(int)$this->config->get('config_language_id').")
                                  WHERE NOT P.product_id IN (SELECT DISTINCT POIP.product_id FROM ".DB_PREFIX."poip_option_image POIP)
                                    AND P.product_id = POV.product_id
                                  ORDER BY P.product_id ASC, POV.option_value_id ASC  
                                  ");
      foreach ($query->rows as $row) {
        $result[] = $row;
      }
  ...
  public function add_product_option_value_image($product_id, $option_value_id, $image, $sort_order_param=false) {
    ...
    if ($query->num_rows) {
      ...
      if (!$query_i->num_rows) {
        ...
        if ($sort_order_param) {
          $sort_order = (int)$sort_order_param;
        } else {
